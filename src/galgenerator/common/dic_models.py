...
class TimestepEmbedder(nn.Module):
    ...
    @staticmethod
    def timestep_embedding(t: torch.Tensor, dim: int, max_period: int):
        """
...
class Downsample(nn.Module):
    def __init__(self, n_feat, out_feat):
        ...
        self.body = nn.Sequential(
            nn.Conv2d(
                n_feat, out_feat // 4, kernel_size=3, stride=1, padding=1, bias=False
            ),
            nn.PixelUnshuffle(2),
        )
        ...
        **kwargs,
        ...
        self.affine = nn.Sequential(
            ...
            nn.Linear(
                ...
                out_features=(in_channels if self.blockconfig else out_channels)
                * self.affinef,
                bias=True,
            ),
        )
...
class DiC(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.x_embedder = OverlapPatchEmbed(
            in_channels, hidden_size * mult_channels[0], bias=True
        )

        self.t_embedder_ls = nn.ModuleList(
            [
                TimestepEmbedder(
                    hidden_size * mult, 256, kwargs.get("max_period", 10000)
                )
                for mult in mult_channels[: self.levels]
            ]
        )
        self.y_embedder_ls = nn.ModuleList(
            [
                LabelEmbedder(num_classes, hidden_size * mult, class_dropout_prob)
                for mult in mult_channels[: self.levels]
            ]
        )
        ...
        # encoder
        for level_idx, mult, next_mult in zip(
            range(stages), mult_channels[:stages], mult_channels[1 : stages + 1]
        ):
            channel_size = int(hidden_size * mult)
            self.enc_blocks.append(
                nn.ModuleList(
                    [
                        U_Block(input_size // (2**level_idx), channel_size, **kwargs)
                        for _ in range(depth[level_idx])
                    ]
                )
            )
        ...
        self.lat_blocks.append(
            nn.ModuleList(
                [
                    U_Block(input_size // (2**stages), channel_size, **kwargs)
                    for _ in range(depth[stages])
                ]
            )
        )

        # decoder
        for level_idx, mult, skip_mult in zip(
            list(range(stages))[::-1],
            mult_channels[-stages:],
            mult_channels[:stages][::-1],
        ):
        ...
        self.output = nn.Conv2d(
            channel_size, channel_size, kernel_size=3, stride=1, padding=1, bias=True
        )
    ...
    def initialize_weights(self):
        ...
        for blocks in self.enc_blocks:
            for block in blocks:
                try:
                    nn.init.constant_(block.conv.affine[-1].weight, 0)
                except:
                    pass
                try:
                    nn.init.constant_(block.conv.affine[-1].bias, 0)
                except:
                    pass
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, y: torch.Tensor | None = None):
        ...
        for idx in range(self.levels):
            ...
            if y is not None:
                ...
                if idx == 0:  # first stage
                    y_emb, y_dropped = self.y_embedder_ls[idx](
                        y, self.training
                    )  # (N, C, 1, 1)
                else:
        ...
        x = self.final_layer(
            x, c_ls[stage_idx - 1]
        )  # (N, T, patch_size ** 2 * out_channels) # stick to last stage
